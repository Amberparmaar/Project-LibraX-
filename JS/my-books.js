...
function loadUserIssuedBooks(userId) {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        if (tableBody) {
            tableBody.innerHTML = `
                <tr>
                    <td colspan="5" class="text-center py-4 text-danger">Failed to load issued books.</td>
                </tr>`;
        }
    });
}

// Authentication Guard (Prevents unauthorized access or unwanted redirects)
onAuthStateChanged(auth, (user) => {
    if (user) {
        loadUserIssuedBooks(user.uid);
    } else {
        // Correct path to login page from member folder
